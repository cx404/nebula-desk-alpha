...
export const JobQueue = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "running": return <Badge className="bg-green-500/20 text-green-400 border-green-500/30">Running</Badge>;
      case "queued": return <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30">Queued</Badge>;
    ...
    <div className="space-y-6">
      ...
      <Card className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
        ...
        <div className="space-y-4">
          {jobs.map((job) => (
            <Card key={job.id} className="bg-white/5 backdrop-blur-xl rounded-xl p-6 border border-white/10">
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-3 h-3 rounded-full ${
                    job.status === 'running' ? 'bg-green-400' :
                    job.status === 'queued' ? 'bg-orange-400' :
                    job.status === 'completed' ? 'bg-blue-400' : 'bg-red-400'
                  }`}></div>
                  <div>
