...
export const FileSync = () => {
  ...
  const storageOptions = [
    { id: "s3", name: "S3 Bucket (ml-datasets)", selected: true },
    { id: "webdav", name: "WebDAV (lab-storage.com)", selected: false },
    { id: "local", name: "Local Workspace", selected: false }
  ];
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case "pending": return <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30">Pending</Badge>;
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "in-progress": return <RefreshCw className="w-4 h-4 text-blue-400 animate-spin" />;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold text-white mb-2">File Sync</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((stat, index) => (
          <Card key={index} className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
              ...
              {storageOptions.map((option) => (
                ...
                >
                  <div className={`w-3 h-3 rounded-full ${
                    selectedStorage === option.id ? 'bg-purple-400' : 'bg-gray-400'
                  }`}></div>
        ...
        <Card className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
          ...
          <div className="space-y-4">
            {syncOperations.map((operation) => (
              <Card key={operation.id} className="bg-white/5 backdrop-blur-xl rounded-xl p-4 border border-white/10">
