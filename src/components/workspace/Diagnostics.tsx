...
export const Diagnostics = () => {
  ...
  const systemMetrics = [
    { name: "CPU Usage", value: 23, color: "bg-blue-500" },
    { name: "Memory", value: 45, color: "bg-green-500" },
    { name: "Disk Usage", value: 78, color: "bg-orange-500" }
  ];
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "passed": return <Badge className="bg-green-500/20 text-green-400 border-green-500/30 text-xs">PASSED</Badge>;
      case "warning": return <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30 text-xs">WARNING</Badge>;
      case "failed": return <Badge className="bg-red-500/20 text-red-400 border-red-500/30 text-xs">FAILED</Badge>;
      default: return <Badge className="bg-gray-500/20 text-gray-400 border-gray-500/30 text-xs">UNKNOWN</Badge>;
    }
  };
        ...
        <Card className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
          ...
          <div className="space-y-4">
            {connectivityTests.map((test) => (
              <Card key={test.id} className="bg-white/5 backdrop-blur-xl rounded-xl p-5 border border-white/10">
