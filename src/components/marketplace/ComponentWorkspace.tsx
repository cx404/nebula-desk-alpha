...
const predefinedFlows: ComponentFlow[] = [
  {
    ...
    name: "机器学习流水线",
    components: [
      { id: "data-prep", name: "数据预处理", icon: <Settings className="w-6 h-6" />, x: 100, y: 100, type: "jupyter", status: "idle" },
      { id: "training", name: "模型训练", icon: <FileText className="w-6 h-6" />, x: 300, y: 100, type: "jupyter", status: "idle" },
      { id: "deploy", name: "模型部署", icon: <Rocket className="w-6 h-6" />, x: 500, y: 100, type: "model-deploy", status: "idle" }
    ],
    connections: [
      { sourceId: "data-prep", targetId: "training", type: "data" },
      { sourceId: "training", targetId: "deploy", type: "control" }
    ]
  },
  {
    id: "dev-workflow",
    name: "开发工作流",
    components: [
      { id: "editor", name: "代码编辑", icon: <FileText className="w-6 h-6" />, x: 100, y: 100, type: "code-editor", status: "idle" },
      { id: "terminal", name: "终端", icon: <Terminal className="w-6 h-6" />, x: 300, y: 100, type: "terminal", status: "idle" },
      { id: "jupyter", name: "测试", icon: <FileText className="w-6 h-6" />, x: 500, y: 100, type: "jupyter", status: "idle" }
    ],
    connections: [
      { sourceId: "editor", targetId: "terminal", type: "control" },
      { sourceId: "terminal", targetId: "jupyter", type: "data" }
    ]
  }
];
...
export const ComponentWorkspace = () => {
  ...
  const runComponent = (componentId: string) => {
    setComponents(prev => prev.map(comp => 
      comp.id === componentId ? { ...comp, status: "running" } : comp
    ));
    
    // 模拟运行
    setTimeout(() => {
      setComponents(prev => prev.map(comp => 
        comp.id === componentId ? { ...comp, status: "idle" } : comp
      ));
  ...
  return (
    <div className="space-y-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-2 text-white">组件工作空间</h2>
          ...
          <Card className="p-4 bg-white/5 backdrop-blur-xl border border-white/10">
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                {selectedComponent && (
                  <Button
                    onClick={() => runComponent(selectedComponent)}
                    size="sm"
                ...
                {components.map((component) => (
                  <div key={component.id} className="absolute">
                    <DraggableComponent
                      id={component.id}
                      name={component.name}
                      icon={typeof component.icon === 'string' ? component.icon : '⚙️'}
                      x={component.x}
                      y={component.y}
